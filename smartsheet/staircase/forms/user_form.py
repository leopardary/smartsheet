...
class user_form(forms.Form):
    ...
    #group_list=[(group.id,group.group_name) for group in Group.objects.all()]#filter(group_name='Staircase')]
    group=forms.ModelChoiceField(label='Group',queryset=Group.objects.filter(group_name='Staircase'))#ChoiceField(label='Group',widget=forms.Select(attrs={'class':'form-control'}),choices=group_list)
