...
class Foup_form(forms.Form):
    ...
    owner=forms.ModelChoiceField(label='Owner',queryset=User.objects.filter(group__group_name='Staircase'))#(label='Owner',widget=forms.Select(attrs={'class':'form-control'}),choices=user_list)
